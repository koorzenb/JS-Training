...
export class Lookups {
    ...
    async processGroups(files) {
        ...
        for (const entry of entries) {
            ...
            const lookup = `${options.directory}\\${resource}Lookups.json`;
            let lookupsSchema = await this.fetchSchema(lookup);
            ...
            for (const key in options.files) {
                ...
                const lookups = await this.extractLookupData(schemaFile);

                if (lookups == null || lookups.length == 0) {
                    continue;
                }
                else {
                    lookupsSchema == null && (lookupsSchema = await this.createLookups());
                }

                await this.addLookupsToSchema(lookups, schemaFile, lookupsSchema);
                await this.deleteLookupData(schemaFile);
                await this.writeSchema(lookupsSchema, lookup, resource);
                await this.writeSchema(schemaFile, path, resource);
            }


            // const createSchema = await this.fetchSchema(create);

            // const lookupsSchema = await this.createLookups();

            // let lookups = await this.extractLookupData(createSchema);
            // await this.addLookupsToSchema(lookups, createSchema, lookupsSchema);

            // const updateSchema = await this.fetchSchema(update);
            // lookups = await this.extractLookupData(updateSchema);
            // await this.addLookupsToSchema(lookups, updateSchema, lookupsSchema);

            // const dashboardSchema = await this.fetchSchema(dashboard);
            // await this.updatePrimaryActions(dashboardSchema);

            // await this.deleteLookupData(createSchema);
            // await this.deleteLookupData(updateSchema);

            // await this.writeSchema(lookupsSchema, lookup, dashboardSchema.resource);
            // await this.writeSchema(createSchema, create, dashboardSchema.resource);
            // await this.writeSchema(updateSchema, update, dashboardSchema.resource);
            // await this.writeSchema(dashboardSchema, dashboard, dashboardSchema.resource);
        }
    }
